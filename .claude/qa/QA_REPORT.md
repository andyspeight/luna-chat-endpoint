...
- **[P0] `api/profile.js` (and the whole `X-Client-Name`-only endpoint class: `luna-brain`, `luna-stats`, `scan-page`) — anonymous cross-tenant read/write.** _Status: deferred by owner decision 2026-06-26 ("leave for now")._ **Deeper diagnosis (confirmed from the front-end):** the originally-proposed fix — adding `validateSession(req.headers.cookie)` — will NOT work, and re-requiring the password will BREAK the live dashboard. Why: `public/dashboard.html` sets `CONFIG.PASSWORD = ''` ("no longer used", line 2873) so it sends an empty `X-Client-Pass`, and it calls the API cross-origin to `luna-chat-endpoint.vercel.app` with no `credentials:'include'`, so the `tg_session` cookie (scoped to `travelify.io`) never reaches the endpoint. The `tg-auth-gate` only protects *loading* the dashboard HTML; it forwards no credential to the API, so these endpoints authenticate on a public business name alone. **Recommendation (needs an architecture choice):** (a) have the gate mint a short-lived signed bearer token the dashboard forwards as `Authorization`, verified server-side — no cookie/CORS coupling; or (b) host the API same-site under `*.travelify.io` + `credentials:'include'` + `Allow-Credentials` so the cookie reaches it, then `validateSession`. Either way also stop returning `emailPlatformApiKey` to the browser. **Why not auto-fixed:** closing it requires coordinated changes at the gate and/or hosting + front-end, not a single-file edit.
