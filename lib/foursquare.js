...
var DEFAULT_HOST = process.env.FOURSQUARE_HOST || 'api.foursquare.com';
...
// results[] with a name, so we read defensively.
function parsePlaces(json) {
  var results = (json && json.results) || [];
  return results.map(function (r) {
    var cat = (r.categories && r.categories[0] && r.categories[0].name) || '';
    return { name: (r.name || '').trim(), category: cat };
...
async function fetchPlaces(destination, opts) {
  ...
  try {
    ...
    var url = buildSearchUrl(host, {
      ...
      radius: opts.radius || 20000, limit: opts.limit || 10,
    ...
    return { ok: true, httpStatus: 200, count: places.length, draft: placesToDraft(places, destination, { max: opts.max || 8 }) };
  } catch (e) {
