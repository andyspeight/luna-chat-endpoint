...
// SECURITY — cross-tenant access is for TRAVELGENIX STAFF ONLY.
//
// The `role` returned by the central auth platform is a role WITHIN the user's own
// organisation, NOT a Travelgenix-wide role. A client who owns their own agency
// account legitimately has role 'owner'. Treating that as a platform superuser
// granted every such client access to EVERY tenant — a live cross-tenant breach
// (a client loaded and edited another agency's Luna configuration).
//
// So role alone is never sufficient: the user must ALSO be on a Travelgenix staff
// domain. Fails closed — an unknown domain gets no cross-tenant access.
const CROSS_TENANT_ROLES = new Set(['owner', 'admin']);
const STAFF_DOMAINS = String(process.env.TG_STAFF_DOMAINS || 'agendas.group,travelgenix.io,travelify.io')
...
function isStaffEmail(email) {
  ...
  return STAFF_DOMAINS.indexOf(e.slice(at + 1)) !== -1;
}

// True only for genuine Travelgenix staff who may act across tenants.
function isCrossTenantUser(role, email) {
  return CROSS_TENANT_ROLES.has(String(role || '').toLowerCase()) && isStaffEmail(email);
}
...
/**
 * Airtable filter matching a client by the name a WIDGET calls itself.
 *
 ...
 * record silently breaks the chat on their live site: the lookup stops matching
 * and the widget can no longer load. That is a real trap — Jamie Wake Travel's
 * account is still named after the demo site it was created from, and renaming
 * it without re-embedding would have taken his chat down.
 *
 * LegacyClientName keeps the old name working until their site is re-embedded.
 * Single value, exact match: no list parsing, so there is nothing to
 * mis-tokenise and no chance of one client's alias matching another.
 *
 * Use this EVERYWHERE a client is looked up by widget-supplied name.
 */
function clientNameFormula(clientName) {
  const n = escFormula(String(clientName || '').toLowerCase());
  return "OR(LOWER({ClientName})='" + n + "',LOWER({LegacyClientName})='" + n + "')";
}
...
async function resolveEntitledClient(atKey, session, clientId) {
  ...
  // Travelgenix STAFF may access any client. A client who is merely 'owner' of
  // their own organisation must NOT — see the note on CROSS_TENANT_ROLES.
  if (isCrossTenantUser(session.role, session.user && session.user.email)) {
...
async function resolveClientByName(atKey, clientName) {
  const recs = await fetchClients(atKey, clientNameFormula(clientName), 1);
  return recs[0] || null;
}
