...
// Returns { ok, httpStatus?, count?, draft?, error? } so callers can see WHY a
// destination produced nothing (bad/inactive key vs genuinely no events).
async function fetchEvents(destination, opts) {
  ...
  try {
    ...
    return { ok: true, httpStatus: 200, count: events.length, draft: eventsToDraft(events, destination, { max: opts.max || 5 }) };
  } catch (e) {
