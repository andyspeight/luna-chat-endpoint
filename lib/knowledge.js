...
function formatKnowledgeForPrompt(items) {
  ...
  var lines = [
    ...
    '',
    'CRITICAL — ANSWERING WHEN MULTIPLE ITEMS MATCH:',
    '  - The items below are sorted by relevance. The FIRST item is the most direct answer to the visitor\'s question.',
    '  - LEAD with the direct answer to what the visitor actually asked. Do not preamble with a related-but-different topic.',
    '  - Example: if the visitor asks about CANCELLATION POLICY and you have both a cancellation answer AND an insurance answer, lead with cancellation. You may briefly note that insurance relates, but only AFTER you have answered the actual question, and only if it adds value.',
    '  - DO NOT pivot to a tangential topic just because it also matched the search. Stay on the question.',
    '',
    ...
    '  - When mentioning specific suppliers, partners, providers, or company names: use ONLY names that appear VERBATIM in the approved answer above. Never volunteer specific brand or company names that are not in the source material — if no provider is named in the source, say "we can suggest a provider" or similar generic phrasing.',
    '  - You MUST emit the [KNOWLEDGE:recXXX] marker for each item you ACTUALLY USE, on its own line at the very end of your reply (before any [BLOCK]). This marker is stripped before the visitor sees it. Always include it. Never invent record IDs — copy them exactly from the items below.',
...
async function trackKnowledgeUsage(atKey, knowledgeRecordIds) {
  ...
  try {
    var clauses = knowledgeRecordIds.slice(0, 50).map(function(id) {
    ...
    for (var i = 0; i < updates.length; i += 10) {
      ...
      // Isolate each batch: one failing/timed-out batch must not abandon the rest,
      // and a non-ok response should be visible rather than silently dropped.
      try {
        ...
        if (!patchRes.ok) {
          console.warn('[knowledge] usage PATCH batch failed:', patchRes.status);
        }
      } catch (e) {
        console.warn('[knowledge] usage PATCH batch error:', e.message);
      }
    }
  } catch (err) {
...
async function upsertKnowledgeGap(atKey, clientRecordId, question, topic, convId, suggestedAnswer) {
  ...
  try {
    ...
    var existingUrl = 'https://api.airtable.com/v0/' + AT_BASE + '/' + GAPS_TABLE
      ...
      + '&maxRecords=10';
    var r = await fetch(existingUrl, { headers: { 'Authorization': 'Bearer ' + atKey }, signal: AbortSignal.timeout(8000) });
