...
function formatKnowledgeForPrompt(items) {
  ...
  var lines = [
    '',
    '## Approved answers',
    'The business owner has provided the following authoritative answers. When a visitor asks about any of these topics, use the approved answer directly — the wording matters because it reflects how this business actually communicates. You can adapt the tone naturally but keep the substance unchanged.',
    'When you use one of these in your reply, include the corresponding [KNOWLEDGE:recXXX] marker on its own line at the very end of your reply (before any [BLOCK]). The marker will be stripped before the visitor sees it. Include one marker per item used. Never invent recordIDs.',
    ''
  ];
  items.forEach(function(item, idx) {
    lines.push('### Item ' + (idx + 1) + ' [KNOWLEDGE:' + item.id + ']');
