...
function buildSearchBody(query, opts) {
  ...
  return {
    ...
    // 'advanced' + raw content gives the grounded checker real page text to
    // judge, not a one-line snippet (which always reads as unverifiable).
...
function parseResults(json) {
  ...
  return results.map(function (r) {
    return {
      ...
      rawContent: (r.raw_content || '').trim()
    };
  }).filter(function (r) { return r.url && (r.content || r.rawContent); });
}
...
async function search(query, opts) {
  ...
  try {
    var r = await fetch(ENDPOINT, {
      method: 'POST',
      // Current Tavily auth is a Bearer token; we also keep api_key in the body
      // for older keys. One of the two satisfies whichever the account uses.
