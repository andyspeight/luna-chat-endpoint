...
// Strict enough that what we accept, SendGrid will accept.
//
// The first version of this was /^[^\s@,]+@[^\s@,]+\.[^\s@,]+$/, copied from the
// notification paths it replaced. That is fine as a last-ditch check on a value
// someone else set, and wrong as the gate on a box a client types into, because
// it waves through things that bounce: "bob@shop.co.uk." with a trailing full
// stop, "<bob@shop.co.uk>" pasted out of Outlook, ".bob@", "bob@shop..co.uk",
// "bob@-shop.co.uk", "bob@shop.c".
//
// That mattered more here than it looks. A value that passes validation is
// stored, and a stored value is non-empty, and a non-empty NotificationEmail
// SUPPRESSES the ContactEmail fallback. So a client pasting an address with a
// stray bracket would have saved it successfully, seen "Saved!", and had every
// enquiry stop arriving with no error anywhere. Silently losing a client's
// leads is worse than the problem this feature was built to solve.
//
// So: a local part of permitted characters with no leading, trailing or
// doubled dot; domain labels that start and end alphanumeric; and a real
// alphabetic TLD of two or more. Anything that now fails is dropped by
// parseList, which means recipients() falls back to ContactEmail rather than
// going quiet — the safe direction.
var LOCAL = "[A-Za-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[A-Za-z0-9!#$%&'*+/=?^_`{|}~-]+)*";
var LABEL = '[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?';
var EMAIL_RE = new RegExp('^' + LOCAL + '@' + LABEL + '(?:\\.' + LABEL + ')*\\.[A-Za-z]{2,}$');
