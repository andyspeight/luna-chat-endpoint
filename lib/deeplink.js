// Deep-link post-processor: make Luna's searches match the site's searches.
//
// Every dl.tvllnk.com link the model writes passes through here on its way to
// the visitor, and two things are corrected: WHERE the search points (the
// curated centre, radius and airport grouping from the site's own geo table)
// and WHAT LANGUAGE the results page comes back in (the Lang parameter).
//
...
//
// The two corrections have different reach. Geography only applies to a search
// that names a place, so a Flights link (which carries airports and no place)
// is left alone. Language applies to EVERY link, Flights included: a Romanian
// agency's visitor should land on a Romanian results page whatever they were
// searching for.
...
// opts.lang, when given, is the deep link's language code (e.g. 'RO').
function rewriteLink(url, opts) {
  ...
  // Language first, because it applies to every search type — including
  // Flights, which returns below before any geography is considered.
  var lang = opts && opts.lang ? String(opts.lang).trim().toUpperCase() : '';
  if (lang && /^[A-Z]{2}$/.test(lang) && p.get('Lang') !== lang) {
    p.set('Lang', lang);
    out.url = u.toString();
    out.changed = out.url !== url;
    out.lang = lang;
  }
