...
async function check(key, max, windowSecs) {
  ...
  // Atomic-ish: INCR, then EXPIRE. EXPIRE refreshes the TTL on every call, giving
  // a sliding window. That's slightly more restrictive than a fixed window (good),
  // at the cost of bursts right at the window edge being caught twice.
