...
async function pipeline(commands) {
  ...
  try {
    ...
    // TEMPORARY DEBUG: log what Upstash actually returned so we can match our parser to it
    console.log('[ratelimit] Upstash raw response:', JSON.stringify(data).slice(0, 300));
    if (!Array.isArray(data)) return null;
    // Upstash returns one of two formats depending on API version:
    //   [{"result":X},{"result":Y}]  ← wrapped
    //   [X, Y]                        ← unwrapped
    // Normalise to the wrapped form so callers can always use .result
...
async function check(key, max, windowSecs) {
  ...
  // Atomic: INCR then EXPIRE (always, not just NX — simpler and always works)
  ...
  if (!results || results.length < 1 || results[0].result === undefined) {
    // Fail-open
    console.warn('[ratelimit] No results from pipeline, failing open for', key, 'raw:', JSON.stringify(results));
    return { allowed: true, remaining: -1, limit: max, resetSecs: windowSecs };
  }

  var count = parseInt(results[0].result, 10);
  if (isNaN(count)) {
    console.warn('[ratelimit] NaN count, failing open. Raw:', JSON.stringify(results));
    return { allowed: true, remaining: -1, limit: max, resetSecs: windowSecs };
  }

  var expireResult = results[1] ? results[1].result : 'missing';
  console.log('[ratelimit]', key, '→ count=' + count + ' max=' + max + ' allowed=' + (count <= max) + ' expire=' + expireResult);
