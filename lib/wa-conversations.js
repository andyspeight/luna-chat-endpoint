...
//   • Existing row: append "\n<role>: <text>" to the transcript, bump
//     LastMessageAt, and ONLY re-open (Status -> new) if the current Status is
//     Closed — otherwise leave Status untouched so we never yank an active agent
//     out of a live conversation.
//
// Field names/choices confirmed against the live base: the "handler" field is
// actually Status (singleSelect: Bot | Waiting | Active | Closed), and
// fldZ38GYN4XbHGl03 is the Summary field which api/conversation.js (the web
// live writer) reuses as the running transcript — we match that for parity.
...
const F = {
  ...
  handler:   'fldYdZq59FCpKQ7Hf', // Status (singleSelect: Bot | Waiting | Active | Closed)
  startedAt: 'fldSoy7BMqyzVb5pp', // StartedAt
  updatedAt: 'fld1GghMiUnAmdtow', // LastMessageAt
  history:   'fldZ38GYN4XbHGl03', // Summary, reused as running transcript (lines "role: text")
