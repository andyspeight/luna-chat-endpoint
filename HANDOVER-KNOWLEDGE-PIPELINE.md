...
- ✅ **#6 grounded Destination Spotlight** — `api/highlights-card.js` now grounds the AI card in verified KB facts via `lib/spotlight-context.js` (real things‑to‑do + destination facts + Q&A). No verified data ⇒ the card stays generic and names nothing it cannot source (no hallucination). On by default; `SPOTLIGHT_GROUNDED=0` reverts. Cache bumped to `highlights:v2:`.
- Still open: a duplicate‑finder for the global Knowledge table. Possible next: extend `fill-gaps`‑style grounding to richer fields; a per‑destination coverage % view; run `embed-index` more often if event freshness in semantic results matters.
